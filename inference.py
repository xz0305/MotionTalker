...
def inference(image_path, driver_path, resume_path, force_retrack=False, device='cuda'):
    ...
    gt_video = (gt_video.squeeze() * 255.0).to(torch.uint8).permute(0, 2, 3, 1)
    torchvision.io.write_video(gt_dump_path, gt_video, fps=25.0)

    merged_images = (merged_images.squeeze() * 255.0).to(torch.uint8).permute(0, 2, 3, 1)
